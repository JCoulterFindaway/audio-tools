...
class FFmpegVersionManager:
    ...
    This is particularly useful for diagnosing corrupt audio files, as
    different
    ...
    def __init__(self, preferred_version: str = None, use_docker: bool = True):
        ...
        Args:
            ...
            use_docker: Whether to prefer Docker-based versions for true
                isolation.
        """
    ...
    def is_docker_image_available(self, version: str) -> bool:
        ...
        """
        if (
            not self.is_docker_available()
            or version not in self.DOCKER_VERSIONS
        ):
            return False
    ...
    def get_binary_path(self, version: str = None) -> str:
        ...
        if self.use_docker and self.is_docker_image_available(version):
            # Special identifier for Docker execution
            return f"docker_ffmpeg_{version}"
        ...
        if version == "system":
            if not shutil.which(binary_path):
                raise FileNotFoundError("System ffmpeg not found in PATH")
        else:
            ...
            if not found_path:
                raise FileNotFoundError(
                    f"FFmpeg {version} not found at {binary_path} or "
                    f"{possible_paths[1]}"
                )
    ...
    def execute_ffmpeg_command(
        self, version: str, command_args: List[str], input_file: str
    ) -> subprocess.CompletedProcess:
        """
        Execute an ffmpeg command using the specified version
        (Docker or native).
        ...
        if self.use_docker and self.is_docker_image_available(version):
            ...
            docker_cmd = [
                ...
                arg.replace(str(input_file), container_file)
                for arg in command_args
            ]

            return subprocess.run(
                docker_cmd, capture_output=True, text=False, check=False
            )
        else:
            ...
            if binary_path.startswith("docker_ffmpeg_"):
                raise RuntimeError(
                    f"Docker not available for version {version}"
                )

            ffmpeg_cmd = [binary_path] + command_args
            return subprocess.run(
                ffmpeg_cmd, capture_output=True, text=False, check=False
            )
    ...
    def list_available_versions(self) -> List[str]:
        ...
        if self._available_versions is None:
            ...
            for version in self.VERSION_PATHS:
                try:
                    # Check Docker version first if enabled
                    if self.use_docker and self.is_docker_image_available(
                        version
                    ):
    ...
    def get_version_info(self, version: str = None) -> Dict[str, str]:
        ...
        Returns:
            Dictionary with version details (version, build_date,
            configuration, etc.)
        """
        version = version or self.preferred_version
        ...
        try:
            ...
            # Check if this is a Docker-based version
            if binary_path.startswith("docker_ffmpeg_"):
                # Use Docker execution for version info
                result = subprocess.run(
                    [
                        "docker",
                        "run",
                        "--rm",
                        self.DOCKER_VERSIONS[version],
                        "-version",
                    ],
                    capture_output=True,
                    text=True,
            ...
            self._version_info_cache[version] = version_info
    ...
    def get_best_available_version(self) -> str:
        """
        Get the best available version, preferring the default but
        falling back.
        ...
        if not available:
            raise RuntimeError(
                "No ffmpeg versions are available on this system"
            )
    ...
    def print_status(self) -> None:
        ...
        if not available_versions:
            ...
            print(
                "\nPlease install ffmpeg using the provided "
                "installation guide."
            )
            return
        ...
        for version in self.VERSION_PATHS.keys():
            ...
            path_str = version_info.get('binary_path', 'N/A')
            print(
                f"{status} {version:10} | {version_str:15} | "
                f"{path_str}{execution_type}"
            )

            if version in available_versions and "error" not in version_info:
                print(
                    f"   └─ {version_info.get('build_info', 'No build info')}"
                )
...
class FFmpegConfig:
    ...
    def __init__(
        self, version: str = None, version_manager: FFmpegVersionManager = None
    ):
        """
...
def get_ffmpeg_binary(version: str = None) -> str:
    ...
    """
    ffmpeg_config = FFmpegConfig(version=version)
    return ffmpeg_config.binary_path
...
if __name__ == "__main__":
    ...
    try:
        default_config = FFmpegConfig.default()
        print(f"Default config: {default_config}")

        version_info = default_config.get_version_info()
        print(f"Version info: {version_info.get('version', 'unknown')}")
